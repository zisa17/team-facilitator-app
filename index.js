...
app.post('/notion/pages', async (req, res) => {
  ...
  try {
    ...
    const d = await r.json();
    if (!r.ok) return res.status(r.status).json(d);
    res.json({ ok: true, id: d.id, url: d.url });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ── Supabase proxy ────────────────────────────────────────────
// Forwards all /supabase/* requests to Supabase REST API
// e.g. GET /supabase/teams → GET https://xxx.supabase.co/rest/v1/teams
...
app.post('/slack', async (req, res) => {
  ...
  if (!webhook) return res.status(400).json({ error: 'No webhook provided' });
  try {
    await fetch(webhook, {
      method: 'POST',
...
app.listen(PORT, () => {
  console.log(`Team Facilitator proxy v6 on port ${PORT}`);
